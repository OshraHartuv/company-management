<template>
  <section class="app" v-if="compId">
    <button class="add-btn btn">
     ...
     <list :fields="deps" @remove="showModal"  v-if="deps && deps.length">
      <!-- <template #thead> -->
        <th>Name</th>
        <th></th>
      <!-- </template> -->
...
<script>
import DeleteModal from "@/components/dep/DeleteModal.vue";
import ReassignModal from "@/components/dep/ReassignModal.vue";
  ...
  methods: {
    ...
    async reassign(emps) {
      try {
        ...
        this.empsToReassign = [];
        this.depsToShow = [];
        this.depToDelete = this.deps.find(dep => dep._id === this.depToDeleteId);
      } catch (err) {
